...
st.write(pd.Series(y_data).value_counts().rename(index=label_encoder.inverse_transform(np.arange(len(label_encoder.classes_)))))

# Split data when the user clicks the "Train Model" button
if train_button:
    with st.spinner("Training model..."):
        ...
        random_search = RandomizedSearchCV(estimator=rf_classifier, param_distributions=param_dist, 
                                          n_iter=10, cv=3, verbose=1, n_jobs=-1, random_state=random_state)
